                ...
                {projects.map((project, index) => {
                    ...
                    return (
                        <ScrollReveal delay={0.1 * project.id + 0.2}>
                            <div
                                key={project.id}
                                className="w-[335px] border border-teal-200 dark:border-none bg-teal-50 dark:bg-black rounded-lg shadow-xs overflow-hidden shadow-teal-300 hover:shadow-lg transition-shadow duration-300"
                                onMouseEnter={() => handleCardHover(index, true)}
                                onMouseLeave={() => handleCardHover(index, false)}
                            >
                                {/* Image Section */}
                                <div className="relative h-45 w-[335px] overflow-hidden">
                                    <motion.img
                                        key={`${project.id}-${currentImageIndex}`}
                                        src={project.img[currentImageIndex]}
                                        alt={`${project.name} - Image ${currentImageIndex + 1}`}
                                        className="w-[334px] h-full object-cover "
                                        loading="lazy"
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
